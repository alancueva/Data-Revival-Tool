...
string ImageMetadata::getFileType() const
{
    return "Imagen - " + imageFormat;
}

string ImageMetadata::extractMetadata()
{
    if (!isValid)
        return "Archivo no válido o no es una imagen compatible";
...
string ImageMetadata::recoverOverwrittenMetadata()
{
    if (!isValid)
        return "No se puede recuperar metadata de un archivo no válido";
...
bool ImageMetadata::analyzeJPEG(ifstream &file)
{
...
bool ImageMetadata::analyzePNG(ifstream &file)
{
    // Restablecemos la posición del archivo
    file.seekg(0, ios::beg);
...
bool ImageMetadata::analyzeTIFF(ifstream &file)
{
...
bool ImageMetadata::analyzeBMP(ifstream &file)
{
...
bool ImageMetadata::analyzeGIF(ifstream &file)
{
